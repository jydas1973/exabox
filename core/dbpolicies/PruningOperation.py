...
NAME:
    PruningOperation - Handle DB Pruning, saving select queries, DELETING all other
...
class ebDBPruningOperation(ebDbOperation):
    # Pruning only supports time based trigger
    @staticmethod
    def mIsTriggerSupported(aEbDbTrigger):
        return isinstance(aEbDbTrigger, ebTimeBasedTrigger)

    # Pruning is implemented by a DBStore method
    @staticmethod
    def mExecute(aEbDbTrigger, aEbFilters=[]):
        return ebGetDefaultDB().mPruneRequests(aEbDbTrigger.mGetFilterData(),
                                               aEbFilters)