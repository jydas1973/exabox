#!/bin/python
#
# $Header: ecs/exacloud/exabox/exatest/cluctrl/shared_methods/tests_create_unmount_lvm.py /main/8 2025/12/18 16:42:14 jfsaldan Exp $
#
...
class ebTestCreateDeleteLVM(ebTestClucontrol):
    ...
    def test_create_lvm_exascale_wipes_existing_volume(self):
        """
        Function to test mCreateImageLVM in ExaScale EDV path
        """

        # Declare variables to use
        _img_path = "/dev/exc/u02_Vmzktrs_1_e811"
        ...
        _cmds = {
            self.mGetRegexDom0(): [
                [
                    ...
                    exaMockCommand(f"/sbin/parted {_img_path} set 1 lvm on"),
                    exaMockCommand(f"kpartx -a -v {_img_path}",
                        aStdout = "add map u02_Vmzktrs_1_e811 (252:11): 0 4194207 linear /dev/loop0 64\n",
                        aRc = 0),
                    exaMockCommand("losetup -a",
                        aStdout = f"/dev/loop0: [64518]:{_sector_size} ({_img_path})",
                        aRc = 0),
                    ...
                    exaMockCommand("lvremove -f VGExaDbDisk.u02_extra.img/LVDBDisk", aStdout = "\n", aRc = 0),
                    exaMockCommand("vgchange -an VGExaDbDisk.u02_extra.img", aStdout = "\n", aRc = 0, aPersist=True),
                    exaMockCommand("vgremove -f VGExaDbDisk.u02_extra.img", aStdout = "\n", aRc = 0, aPersist=True),
                    exaMockCommand("vgremove VGExaDbDisk.u02_extra.img --force", aStdout = "\n", aRc = 0, aPersist=True),
                    exaMockCommand("pvremove /dev/mapper/u02_Vmzktrs_1_e811 --force --force <<< y", aStdout = "\n", aRc = 0),
                    exaMockCommand("kpartx -d -v /dev/exc/u02_Vmzktrs_1_e811", aStdout = "\n", aRc = 0),
                    exaMockCommand("losetup -d /dev/exc/u02_Vmzktrs_1_e811", aStdout = "\n", aRc = 0),
                    exaMockCommand("/usr/sbin/udevadm settle ;  pvscan --cache", aStdout = "\n", aRc = 0),
                ]
            ]
        }
