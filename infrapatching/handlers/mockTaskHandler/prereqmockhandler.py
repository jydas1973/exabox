#
# $Header: ecs/exacloud/exabox/infrapatching/handlers/mockTaskHandler/prereqmockhandler.py /main/2 2024/11/04 07:22:22 emekala Exp $
#
# prereqmockhandler.py
#
# Copyright (c) 2020, 2024, Oracle and/or its affiliates.
#
#    NAME
#      prereqmockhandler.py - Wrapper Patching API to expose patch prereq operation
#
#    DESCRIPTION
#      This module has mExecute method for Exadata patch operation on
#      dom0, Cell, domU and Switch.
#
#    NOTES
...
#    nmallego    11/11/21 - Bug33521580 - Update correct task type for prereq
...
class PreReqMockHandler(TaskMockHandler):

    def __init__(self, *initial_data, **kwargs):
        super(PreReqMockHandler, self).__init__(*initial_data, **kwargs)
        mRegisterInfraPatchingHandlers(INFRA_PATCHING_HANDLERS, [TASK_PREREQ_CHECK], self)
        self.mPatchLogInfo("PreReqMockHandler")
    ...
    def mExecute(self):
        ...
        # Infra patching have a capability to run patching on all targets at the same time with target type PATCH_ALL,
        # but not implemented as part of refactor code.
        for _ttype in [PATCH_CELL, PATCH_DOM0, PATCH_DOMU, PATCH_IBSWITCH, PATCH_ROCESWITCH]:
            ...
            if _targetHandler:
                _rc = _targetHandler.mHandleExistingPatchMgrSessionsOnRetry()
                if _rc == PATCH_SUCCESS_EXIT_CODE:
                    _rc, _no_action_taken = _targetHandler.mPreCheck()
                else:
                    self.mPatchLogError("Error happened while Draining PatchMgr session during retry.")
                self.mPatchLogInfo(f"Task: {TASK_PREREQ_CHECK} - Type: {_ttype}\t\t[ ret_code = {_rc} ]")
        ...
        if _targetHandler is None:
            raise Exception("No Target handler specified for PreCheck")
        else:
            if _rc == PATCH_SUCCESS_EXIT_CODE and _no_action_taken == len(self.mGetTargetTypes()):
                ...
                _suggestion_msg = f"Task: {TASK_PREREQ_CHECK} - Type: {self.mGetTargetTypes()} completed successfully."
                self.mAddError(_rc, _suggestion_msg)
