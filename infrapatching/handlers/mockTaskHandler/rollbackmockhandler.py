#
# $Header: ecs/exacloud/exabox/infrapatching/handlers/mockTaskHandler/rollbackmockhandler.py /main/2 2024/11/04 07:22:22 emekala Exp $
#
# rollbackmockhandler.py
#
# Copyright (c) 2020, 2024, Oracle and/or its affiliates.
#
#    NAME
#      rollbackmockhandler.py - Wrapper Patching API to expose rollback operation
#
#    DESCRIPTION
#      This module has mExecute method for Exadata rollback operation on
...
#    nmallego    11/10/21 - Bug33521580 - Run cell rollback before dom0
...
class RollBackMockHandler(TaskMockHandler):

    def __init__(self, *initial_data, **kwargs):
        super(RollBackMockHandler, self).__init__(*initial_data, **kwargs)
        mRegisterInfraPatchingHandlers(INFRA_PATCHING_HANDLERS, [TASK_ROLLBACK], self)
        self.mPatchLogInfo("RollBackMockHandler")
    ...
    def mExecute(self):
        ...
        for _ttype in [PATCH_CELL, PATCH_DOM0, PATCH_DOMU, PATCH_IBSWITCH, PATCH_ROCESWITCH]:
            ...
            if _targetHandler:
                ...
                if _rc == PATCH_SUCCESS_EXIT_CODE:
                     _rc, _no_action_taken = _targetHandler.mRollBack()
                else:
                    self.mPatchLogError("Error happened while Draining PatchMgr session during retry.")
                self.mPatchLogInfo(f"Task: {TASK_ROLLBACK} - Type: {_ttype}\t\t[ ret_code = {_rc} ]")
        ...
        if _targetHandler is None:
            raise Exception("No Target handler specified for RollBack")
        else:
            if _rc == PATCH_SUCCESS_EXIT_CODE and _no_action_taken == len(self.mGetTargetTypes()):
                ...
                _suggestion_msg = f"Task: {TASK_ROLLBACK} - Type: {self.mGetTargetTypes()} completed successfully."
                self.mAddError(_rc, _suggestion_msg)
