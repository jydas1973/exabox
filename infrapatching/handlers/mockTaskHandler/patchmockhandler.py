#
# $Header: ecs/exacloud/exabox/infrapatching/handlers/mockTaskHandler/patchmockhandler.py /main/2 2024/11/04 07:22:22 emekala Exp $
#
# patchmockhandler.py
#
...
#
#    NAME
#      patchmockhandler.py - Wrapper Patching API to expose patch operation
#
#    DESCRIPTION
...
#    nmallego    11/11/21 - Bug33521580 - Update correct task type for patch 
...
class PatchMockHandler(TaskMockHandler):

    def __init__(self, *initial_data, **kwargs):
        super(PatchMockHandler, self).__init__(*initial_data, **kwargs)
        self.mPatchLogInfo("PatchMockHandler")
    ...
    def mExecute(self):
        ...
        for _ttype in [PATCH_CELL, PATCH_DOM0, PATCH_DOMU, PATCH_IBSWITCH, PATCH_ROCESWITCH]:
            ...
            if _targetHandler:
                ...
                if _rc == PATCH_SUCCESS_EXIT_CODE:
                    _rc, _no_action_taken = _targetHandler.mPatch()
                else:
                    self.mPatchLogError("Error happened while Draining PatchMgr session during retry.")
                self.mPatchLogInfo(f"Task: {TASK_PATCH} - Type: {_ttype}\t\t[ ret_code = {_rc} ]")
        ...
        if _targetHandler is None:
            raise Exception("No Target handler specified for Patch")
        else:
            if _rc == PATCH_SUCCESS_EXIT_CODE and _no_action_taken == len(self.mGetTargetTypes()):
                ...
                _suggestion_msg = f"Task: {TASK_PATCH} - Type: {self.mGetTargetTypes()} completed successfully."
                self.mAddError(_rc, _suggestion_msg)
