...
#                            TARGET ELU VERSION AND HAS ELU TYPE DURING
#                            POSTCHECK
...
#                           INCLUDE DETAILED DOMU STARTUP CHECK
#    sdevasek    10/18/24 - ENH 37180312 - DO NOT RUN DBHEALTHCHECKS ON
...
class DomUHandler(TargetHandler):

    def __init__(self, *initial_data, **kwargs):
        # invoking the __init__ of the parent class
        super(DomUHandler, self).__init__(*initial_data, **kwargs)
        ...
        # VM cluster can contain only vms of homogeneous type vm(i.e it can not contain some autonomous and some non-autonomous
        self.__is_autonomous_vm_cluster = False
        ...
        if self.mGetInfrapatchExecutionValidator().mCheckCondition('mIsLocalHostLaunchNode'):
            ...
            if _launch_node_type == None:
                _launch_node_type = LAUNCHNODE_TYPE_MANAGEMENT_HOST
            self.__domu_patch_base_dir = mGetLaunchNodeConfig(_launch_node_type, 'patch_base')
        else:
            self.__domu_patch_base_dir = mSetFromEnv(default=DOMU_LAUNCH_NODE_PATCH_BASE,
                                                     aEnvVariable="EXACLOUD_PATCH_PAYLOAD_BASE",
                                                     aErrorOn=["/tmp", "/tmp/"])
        self.mSetIsVMClusterAutonomous()
    ...
    def mGetIsVMClusterAutonomous(self):
        return self.__is_autonomous_vm_cluster

    def mSetIsVMClusterAutonomous(self):
        _node_list = self.mGetCustomizedDomUList()
        if _node_list:
            _autonomous_nat_hostname_vm_list = [_autonomous_vm for _, _dom0_vm_list in self.mGetAutonomousVMList() for
                                                _autonomous_vm in _dom0_vm_list]
            _autonomous_customer_hostname_vm_list = self.mGetDomUCustomerHostNamesforDomuNatHostNames(_autonomous_nat_hostname_vm_list)
            _first_vm_name_from_cluster = self.mGetDomUCustomerNameforDomuNatHostName(_node_list[0])
            if _autonomous_customer_hostname_vm_list and _first_vm_name_from_cluster and _first_vm_name_from_cluster in _autonomous_customer_hostname_vm_list:
                self.__is_autonomous_vm_cluster = True
                self.mPatchLogInfo("This VM cluster is autonomous vm cluster")
    ...
    def mPreCheck(self):
        ...
        try:
            ...
            # Perform ELU based domu prechecks.
            if self.mIsElu():
            ...
            if len(_discarded) > 0:
                ...
                if _ret != PATCH_SUCCESS_EXIT_CODE:
                    self.mPatchLogError(f"Exiting since Return code from mCheckIdemPotency in domU is {_ret} ")
                    return _ret , _no_action_taken
            ...
            _ret = self.mGetCRSHelper().mCheckandRestartCRSonAllDomUWithinCluster()
            if _ret != PATCH_SUCCESS_EXIT_CODE:
                return _ret, _no_action_taken
            ...
            _node_to_patch_nodes = self.mGetDomUToPatchDomU()
            ...
            _nodes_to_patch_except_initial = list(set(_list_of_nodes) -
                                                  set([self.mGetDomUToPatchDomU()]))
            _initial_node_list = [self.mGetDomUToPatchDomU()]
            ...
            _ec_node_precheck = self.mPerformPrecheckonDomU(
            ...
            if len(self.mGetExternalLaunchNode()) == 0:
                _ec_initial_node_precheck = self.mPerformPrecheckonDomU(
            ...
            if _child_request_error_already_exists_in_db:
                ...
                _suggestion_msg = "Exception detected during VM OS precheck operation."
    ...
    def mPerformEluPreCheck(self):
        """
        Does the setup, filter the nodes to perform ELUpre-check,
        idempotency check , custom check and then run the pre-check
        of dom0's
        Return codes:
           1) ret -->
        ...
        try:
            ...
            '''
             Precheck with Elu option - applypending not supported by patchmgr and the following
             error will be observed and is returned at this stage.

             ^[[40;1;31mERROR  ^[[0m: Incorrect Exadata Live Update target specified ! Valid options are highcvss, allcvss or full
            '''
            if (self.mGetEluOptions() and self.mGetEluOptions().lower() in [ "applypending" ]):
                self.mPatchLogWarn(f"Elu option - {self.mGetEluOptions().lower()} not support during patchmgr precheck, no further action performed.")
                return _ret, _no_action_taken

            '''
             Ignore the precheck and return PATCH_SUCCESS_EXIT_CODE if all DomUs are
            ...
            if len(self.mGetEluTargetVersiontoNodeMappings()) == 1 and str(list(self.mGetEluTargetVersiontoNodeMappings().keys())[0]) in INVALID_REGISTERED_PATCH_VERSIONS:
                self.mPatchLogWarn(f"Input target version - {INVALID_REGISTERED_PATCH_VERSIONS} is invalid. Prechecks will be skipped on all DomUs - {str(_list_of_elu_nodes)}.")
                return _ret, _no_action_taken

            for _elu_versions, _list_of_elu_nodes in self.mGetEluTargetVersiontoNodeMappings().items():
                '''
                  "ELUTargetVersiontoNodeMappings": {
                     "24.1.15.0.0.240605": "scaqan17celadm03.oraclecloud.internal, scaqan17adm02.oraclecloud.internal, scaqan17adm01.oraclecloud.internal"
                   }
                '''
                self.mPatchLogInfo(f"\n\nTarget ELU version used - {_elu_versions}.")

                '''
                 During an ELU domu infra patch operation, cp will always send customer hostname
                 and Infra patching expects NAT hostname for the patching to work.
                '''
                _list_of_elu_nodes = self.mGetDomUNatHostNamesforDomuCustomerHostNames(_list_of_elu_nodes)
                self.mSetCustomizedDomUList(_list_of_elu_nodes)

                self.mPatchLogInfo(f"List of nodes where ELU precheck will run is {_list_of_elu_nodes}.\n\n")
                ...
                '''
                 ...
                 For domu ELU patching , currently self.mGetCustomizedDomUList() will
                 always have one node.
                '''
                if self.mGetTargetVersion() in INVALID_REGISTERED_PATCH_VERSIONS:
                    self.mPatchLogError(f"Input target version - {INVALID_REGISTERED_PATCH_VERSIONS} is invalid. Prechecks will be skipped on the current set of nodes - {str(_list_of_elu_nodes)}.")
                    _ret = INVALID_TARGET_VERSION
                ...
                # Precheck is not a fail fast operation and the precheck will be run on other DomUs.
                ...
                if self.mGetInfrapatchExecutionValidator().mCheckCondition('mIsPatchBaseMarkerFileApplicable'):
                    if self.mGetInfrapatchExecutionValidator().mCheckCondition('mIsManagementHostLaunchNodeForDomU'):
                        _ret = self.mCreateMarkerFileInPatchBase(self.mGetDomUPatchBaseAfterUnzip(),
                                                                 aNode=self.mGetDomUToPatchDomU())
                    else:
                        _ret = self.mCreateMarkerFileInPatchBase(self.mGetDomUPatchBaseAfterUnzip())
                    if _ret != PATCH_SUCCESS_EXIT_CODE:
                        return _ret, _no_action_taken
                ...
                # Get customized list of nodes
                _ret, _suggestion_msg, _list_of_nodes, _discarded = self.mFilterNodesToPatch(_list_of_elu_nodes, PATCH_DOMU, TASK_PATCH)
                if _ret != PATCH_SUCCESS_EXIT_CODE:
                    self.mAddError(_ret, _suggestion_msg)
                    return _ret, _no_action_taken
                ...
                '''
                 ...
                 only on the input node provided.
                '''
                _list_of_nodes = self.mGetDomUCustomerHostNamesforDomuNatHostNames(_list_of_nodes)                
                _nodes_to_patch_except_initial = list(set(_list_of_nodes) -
                                                      set([self.mGetDomUToPatchDomU()]))
                _initial_node_list = [self.mGetDomUToPatchDomU()]
                _cns_string = CNS_DOMU_PATCHER
                ...
                # Node Failure Case
                    ...
                    if (_ec_initial_node_precheck != PATCH_SUCCESS_EXIT_CODE and _ec_node_precheck != PATCH_SUCCESS_EXIT_CODE):
                        _ret = DOMU_PRECHECK_EXECUTION_FAILED_ERROR
                        _suggestion_msg = "Patch Prereq check failed on multiple domU nodes"
                        self.mAddError(_ret, _suggestion_msg)
                    elif (_ec_node_precheck != PATCH_SUCCESS_EXIT_CODE):
                        self.mPatchLogError("_ec_node_precheck failed")
                        _ret = _ec_node_precheck
    ...
    def mPerformPrecheckonDomU(self, aNode, aTargetType, aListOfNodesToPatch,
                             aDiscardedNodeList, aPatchInitNode=False):
        """
         The mPerformPrecheckonDomU method is responsible for performing pre-checks
        ...
        if self.mIsElu():
            aNode = self.mGetDomUCustomerNameforDomuNatHostName(aNode)
            aListOfNodesToPatch = list(set(self.mGetDomUCustomerHostNamesforDomuNatHostNames(aListOfNodesToPatch)))
        ...
        _patchMgrObj = InfraPatchManager(aTarget=PATCH_DOMU, aOperation=TASK_PREREQ_CHECK, aPatchBaseAfterUnzip=self.mGetDomUPatchBaseAfterUnzip(),
        ...
        _patchMgrObj.mSetIsoRepo(aIsoRepo=self.mGetDomUPatchZip2Name())
        ...
        if len(aListOfNodesToPatch) > 0:
            ...
            if _exit_code != PATCH_SUCCESS_EXIT_CODE:
                if self.mGetSingleVMHandler():
                    self.mGetSingleVMHandler().mGetPatchMgrDiagFilesForSingleVM(self, _node_patcher,
                                                                                aListOfNodesToPatch,
    ...
    def mOneOff(self):
        ...
        try:
            ...
            if _child_request_error_already_exists_in_db:
                ...
                _suggestion_msg = f"Exception in Running DomU OneOff Plugin {str(e)}"
    ...
    def mPatch(self):
        ...
        try:
            ...
            '''
             ...
             Sample json with the version and corresponding domU details.

              "ELUTargetVersiontoNodeMappings": {
                     "22.1.15.0.0.240605": "scaqan17dv01.oraclecloud.internal",
                     "23.1.13.0.0.240605": "scaqan17dv02.oraclecloud.internal"
              }
              ...
              Domu Elu patching is performed one node at a time and the below loop
              break to set the taregt version when the customer hostname passed
              matches the self.mGetCustomizedDomUList() passed as part of the
              payload.
            '''
            if self.mIsElu():
                for _target_version, _domu_hostnames in self.mGetEluTargetVersiontoNodeMappings().items():
                    '''
                     During an ELU domu infra patch operation, cp will always send customer hostname
                     and Infra patching expects NAT hostname for the patching to work.
                    '''
                    _domu_hostnames = self.mGetDomUNatHostNamesforDomuCustomerHostNames(_domu_hostnames)

                    if len(self.mGetCustomizedDomUList()) == 1 and self.mGetCustomizedDomUList()[0] in _domu_hostnames:
                        '''
                         Elu input json can contain a version - 0.0.0.0.0.0
                         and is not a valid target version and must be treated
                        ...
                        if _target_version in INVALID_REGISTERED_PATCH_VERSIONS:
                            self.mPatchLogError(f"Input target version - {_target_version} is invalid. Patch will be skipped on the current set of nodes - {str(self.mGetCustomizedDomUList())} and the final error code for patch will be marked failure.")
                            _ret = INVALID_TARGET_VERSION
                            return _ret, _no_action_taken
                        else:
            ...
            if not self.mIsElu() and (not self.mGetDomULocalPatchZip() or not self.mGetDomULocalPatchZip2()):
            ...
            _launch_nodes = [self.mGetDomUToPatchDomU(), self.mGetDomUToPatchInitialDomU()]
            '''
             In case of ELU patch already applied, Domu will be considered
             already upto date and will be considered as part of the 
             discarded node list.
            '''
            if self.mIsElu() and self.mGetEluOptions() and self.mGetEluOptions().lower() == "applypending":
                _list_of_nodes = self.mGetCustomizedDomUList()
                self.mPatchLogInfo(f"Elu options passed is {self.mGetEluOptions()} and {str(_list_of_nodes)} will be passed as an input to patchmgr command to apply outstanding work items.")
            else:
                _ret, _suggestion_msg, _list_of_nodes, _discarded = self.mFilterNodesToPatch(self.mGetCustomizedDomUList(), PATCH_DOMU, TASK_PATCH)
                if _ret != PATCH_SUCCESS_EXIT_CODE:
                    self.mAddError(_ret, _suggestion_msg)
                    return _ret, _no_action_taken
            ...
            #   post-checks.
            # Returns NO_ACTION_REQUIRED if no domUs were patched or rolled back
            #
            ...
            if self.mIsElu():
                if _operationStyle == OP_STYLE_ROLLING:
                    _ret = self.mPatchRollbackDomUsRolling(
                        self.mGetBackUpMode(),
                        aNodePatcherAndPatchList=_node_patcher_and_node_patch_list,aRollback=False, aListOfEluNodes=_domu_hostnames)
            ...
            if _child_request_error_already_exists_in_db:
                ...
                _suggestion_msg = "Exception detected during VM OS patch operation."
    ...
    def mRollBack(self):
        ...
        try:
            ...
            # Rollback can be terminate early in case of Elu option passed is applypending.
            if self.mGetEluOptions() and self.mGetEluOptions().lower() == "applypending":
                self.mPatchLogInfo(f"Elu options passed is {self.mGetEluOptions()} and is not applicable during {self.mGetTask()}. No further action performed.")
                return _ret, _no_action_taken
            ...
            if _child_request_error_already_exists_in_db:
                ...
                _suggestion_msg = "Exception detected during VM OS rollback operation."
                _ret = DOMU_PATCH_REQUEST_EXCEPTION
                self.mAddError(_ret, _suggestion_msg)
    ...
    def mPatchImageBackupDomUNode(self, aNode, aListOfNodesToPatch,
        ...
        # create patchmgr object with bare minimum arguments
        _patchMgrObj = InfraPatchManager(aTarget=PATCH_DOMU, aOperation=TASK_BACKUP_IMAGE, aPatchBaseAfterUnzip=self.mGetDomUPatchBaseAfterUnzip(),
                                         aLogPathOnLaunchNode=self.mGetPatchmgrLogPathOnLaunchNode(), aHandler=self)
    ...
    def mImageBackup(self):
        ...
        try:
            ...
            if _child_request_error_already_exists_in_db:
                ...
                _suggestion_msg = "Exception detected during VM backup operation."
    ...
    def mPostCheck(self):
        ...
        try:
            ...
            if _child_request_error_already_exists_in_db:
                ...
                _suggestion_msg = "Exception detected during VM postcheck operation."
    ...
    def mSetEnvironment(self):
        ...
        if self.mIsElu():
            '''
             ...
             For Example - if the patch path passed is of the QMR version format and there is a minmatch in the metdata details.
             It has to be modified to /scratch/araghave/ecra_installs/abhi/mw_home/user_projects/domains/exacloud/PatchPayloads/23.1.11.0.0.240605/DomuYumRepository/exadata_ol7_23.1.11.0.0.240605_Linux-x86-64.zip
             and the ELU version is - 24.1.14.0.0.250706 
             It has to be modified to /scratch/araghave/ecra_installs/abhi/mw_home/user_projects/domains/exacloud/PatchPayloads/24.1.14.0.0.250706/DomuYumRepository/exadata_ol8_24.1.14.0.0.250706_Linux-x86-64.zip
             to pick the current ELU patch path.
            '''
            _patch_stage_path = os.path.dirname(os.path.dirname(os.path.dirname(self.__domu_local_patch_zip2)))
            ...
            try:
                '''
                 ...
                 to reflect ELU. In case of Domu ELU, even though
                 exasplice set to yes, Target version passed is of the
                 ELU patch format.
                '''
                if re.match(_elu_patch_pattern, self.mGetTargetVersion()):
                    ...
                    _parent_patch_path = os.path.join(_patch_stage_path, self.mGetTargetVersion() + '/DomuYumRepository')
                        ...
                        for _patch_file in _list_of_all_patch_files:
                            if self.mIsKvmEnv():
                                if "ovs" not in _patch_file:
                                    self.__domu_local_patch_zip2 = os.path.join(_parent_patch_path, _patch_file)
                                    break
                            else:
        ...
        if self.mGetDomULocalPatchZip() and self.__domu_local_patch_zip2:
            self.mPatchLogInfo(f"Domu local patch zip file name {self.mGetDomULocalPatchZip()}")
            ...
            self.__domu_patch_zip_name = \
                self.mGetDomULocalPatchZip().split("/")[-1]
            ...
            self.__domu_patch_base = self.__domu_patch_base_dir + \
                                     self.mGetDomUPatchZipName() + "_" + \
                                     self.mGetDomUPatchZip2Name() + "/"
            self.__domu_patch_zip = self.mGetDomUPatchBase() + \
                                    self.mGetDomUPatchZipName()
            self.__domu_patch_base_after_unzip = (self.mGetDomUPatchBase() +
                                                  mGetFirstDirInZip(self.mGetDomULocalPatchZip()))
            self.__domu_patchmgr = self.mGetDomUPatchBaseAfterUnzip() + \
                                   "patchmgr"
            self.__domu_patch_zip_size_mb = \
                int(os.path.getsize(self.mGetDomULocalPatchZip())) >> 20
            self.__domu_patch_zip2_size_mb = \
                int(os.path.getsize(self.__domu_local_patch_zip2)) >> 20
            self.__domu_patch_necessary_space_mb = \
                (self.mGetDomUPatchZipSizeMB() + \
                 self.mGetDomUPatchZip2SizeMB() + \
                 self.mGetExadataPatchWorkingSpaceMB())

            # Set current patch. Information necessary to update status in db
            self.mSetCurrentTargetType(PATCH_DOMU)

            _ret, _launchNodes = self.mSetLaunchNodeToPatchOtherDomuNodes()
            if _ret != PATCH_SUCCESS_EXIT_CODE:
                return _ret
            ...
            self.mSetPatchmgrLogPathOnLaunchNode(
                self.mGetDomUPatchBaseAfterUnzip() + "patchmgr_log_" + self.mGetMasterReqId())
            ...
            '''
            _list_of_elu_nodes_with_customer_hostnames = []
            if self.mIsElu():
                _list_of_elu_nodes_with_customer_hostnames = self.mGetDomUCustomerHostNamesforDomuNatHostNames(self.mGetCustomizedDomUList())
                _nodes_to_patch_except_initial = list(
                    set(_list_of_elu_nodes_with_customer_hostnames) - set([self.mGetDomUToPatchDomU()]))
            else:
                _nodes_to_patch_except_initial = list(
                    set(self.mGetCustomizedDomUList()) - set([self.mGetDomUToPatchDomU()]))

            _initial_node_list = [self.mGetDomUToPatchDomU()]
            _initial_node = self.mGetDomUToPatchDomU()
            ...
            '''
            _launch_nodes = [self.mGetDomUToPatchDomU()]
            if _next_node:
                _launch_nodes.append(_next_node)

            self.mSetPatchStatesBaseDir(os.path.join(self.mGetDomUPatchBaseAfterUnzip(), "patch_states_data"))
            ...
            if self.mIsExacloudPluginEnabled():
                self.mGetPluginHandler().mSetPluginsLogPathOnLaunchNode(
                    self.mGetDomUPatchBaseAfterUnzip() + "plugins_log_" + self.mGetMasterReqId())
            ...
            if self.mGetSingleVMHandler():
                ...
                #Incase of SingleVM clusters, passwordless ssh verification should be performed using NAT hostname, VMs are reachable 
                #only with NAT hostnames in production
                #This is because the external launch node in single vm patching, is  not in the same flat subnet as the exadata host 

                _nodes_to_patch_except_initial = self.mGetSingleVMHandler().mConfigureSSHForSingleVM(_initial_node, _nodes_to_patch_except_initial,
                                                                    _ssh_env_setup, self)

            else:
                _ssh_env_setup.mSetSSHPasswordless(_initial_node, _nodes_to_patch_except_initial, aKeyComment=EXAPATCHING_KEY_TAG)
            ...
            #
            if _next_node:
                _ssh_env_setup.mSetSSHPasswordless(_next_node, _initial_node_list, aKeyComment=EXAPATCHING_KEY_TAG)
    ...
    def mCheckIdemPotency(self, aDiscarded):
        ...
        _patchMgrObj = InfraPatchManager(aTarget=PATCH_DOMU, aOperation=self.mGetTask(), aPatchBaseAfterUnzip=self.mGetDomUPatchBaseAfterUnzip(),
                                         aLogPathOnLaunchNode=self.mGetPatchmgrLogPathOnLaunchNode(), aHandler=self)
    ...
    def mSetLaunchNodeToPatchOtherDomuNodes(self):
        ...
        _local_patch_zip = self.mGetDomULocalPatchZip()
        _patch_zip_name = self.mGetDomUPatchZipName()
        _patch_zip_size_mb = self.mGetDomUPatchZipSizeMB()
        _patch_base = self.mGetDomUPatchBase()
        _patch_zip = self.mGetDomUPatchZip()
        _patchmgr = self.mGetDomUPatchMgr()
        _patch_necessary_space_mb = self.mGetDomUPatchNecessarySpaceMB()
        _local_patch_zip2 = self.__domu_local_patch_zip2
        _patch_base_after_unzip = self.mGetDomUPatchBaseAfterUnzip()
        if len(_external_launch_nodes) > 0:
            ...
            if len(_launch_node_candidates) > 0:
                if self.mGetInfrapatchExecutionValidator().mCheckCondition('mIsCpsLaunchNodeForDomU'):
                    ...
                    _selected_launch_node = self.mSetLaunchNodeAsPatchBaseForLocalNode(
                        ...
                        aSingleVmHandler=self.mGetSingleVMHandler())
                else:
    ...
    def mPostDomUPatchCheck(self, aDomU, aPrePatchVersion,
        ...
        *check if the crs is up. Done by the patchmgr.
        ...
        self.mPatchLogInfo(f"Start verifying every {DOM0_DOMU_ONLINE_STATUS_CHECK_SLEEP_IN_SECONDS} seconds if a DomU: [{aDomU}] is reachable with a timeout of {_timeout_for_domu_up} seconds")
    ...
    def mPatchRollbackDomUsRolling(self, aBackupMode, aNodePatcherAndPatchList, aRollback, aListOfEluNodes=[]):
        """
        ...
        _initial_node_list = [self.mGetDomUToPatchDomU()]
        _cns_string = CNS_DOMU_PATCHER
        _node_patch_base_after_unzip = self.mGetDomUPatchBaseAfterUnzip()
        _node_patch_zip2_name = self.mGetDomUPatchZip2Name()
        ...
        for _node_patcher, _node_patch_list in aNodePatcherAndPatchList:
            ...
            for _node_to_patch in _node_patch_list:
                ...
                if (self.mGetTask() in [TASK_PATCH] and self.mGetEluOptions() and self.mGetEluOptions().lower() in [ "highcvss", "allcvss", "full" ]):
                    ...
                    _rc = self.mGetCRSHelper().mCheckCrsIsEnabled(_node_to_patch)
                    if _rc in [ DOMU_INVALID_CRS_HOME, CRS_COMMAND_EXCEPTION_ENCOUNTERED ]:
                        ...
                        if _enable_health_checks_from_cp and not self.mGetIsVMClusterAutonomous():
                            ...
                            if _enable_cdb_downtime_check:
                                ...
                                _rc, _ = self.mGetCRSHelper().mExecuteInfraPreSanityCheck(_node_to_patch)
                                if _rc != PATCH_SUCCESS_EXIT_CODE:
                            ...
                            if _enable_pdb_downtime_check or _enable_pdb_degradation_check:
                                ...
                                if _rc == PATCH_SUCCESS_EXIT_CODE and _enable_pdb_downtime_check:
                                    ...
                                    _rc, _ = self.mGetCRSHelper().mDetectPDBDowntime(_node_to_patch)
                ...
                if _patchmgr_session_exit == PATCH_SUCCESS_EXIT_CODE:  # No patchmgr session found in any of the nodes,
                    ...
                    if self.mGetInfrapatchExecutionValidator().mCheckCondition('mIsCpsLaunchNodeForDomU'):
                        '''
                         Execute the patchmgr command only in case of a QMR/ELU patching
                         and elu option is not applypending.
                        '''
                        if not self.mGetEluOptions() or (self.mGetEluOptions() and self.mGetEluOptions().lower() != "applypending"):
                            # set the launch node and execute patchmgr cmd
                    ...
                    if self.mGetTask() in [ TASK_PATCH ] and self.mIsElu() and self.mGetTask() in [ TASK_PATCH ] and self.mGetEluOptions() and self.mGetEluOptions().lower() == "applypending":
                        ...
                        if _outstanding_work_apply_flag:
                            ...
                            self.mPatchLogInfo(f"Image outstanding items is skipped on nodes - {_node_to_patch} as it is not applicable in the current case.")
                            break
    ...
    def mPatchRollbackDomUsNonRolling(self, aBackupMode, aNodePatcherAndPatchList, aRollback, aListOfEluNodes=[]):
        """
                ...
                if _patchmgr_session_exit == PATCH_SUCCESS_EXIT_CODE:  # No patchmgr session found in any of the nodes, so re-execute
                    ...
                    if self.mGetTask() in [ TASK_PATCH ] and self.mIsElu() and self.mGetTask() in [ TASK_PATCH ] and self.mGetEluOptions() and self.mGetEluOptions().lower() == "applypending":
                        ...
                        if _outstanding_work_apply_flag:
                            ...
                            self.mPatchLogInfo(f"Image outstanding items is skipped on node - {self.mGetCustomizedDomUList()[0]} as it is not applicable in the current case.")
                            break
