...
#    remamid     08/07/25 - rollback 37916448 txn
#    araghave    07/30/25 - Enh 38114577 - PROVIDE A MECHANISM THROUGH ECRACLI
#                           TO UPDATE PATCHSWITCHTYPE OPTIONS TO PATCH SWITCHES
#    jyotdas     07/29/25 - Bug 38235778 - Host updater performs ecra
#                           switchover and this is causing patching to fail
#    avimonda    07/24/25 - Bug 38177025 - GUEST OS PATCHING FAILING WITHE ECRA
#                           ERROR
#    sdevasek    07/22/25 - Bug 38095199 - ERRORCODE IS COMING AS NULL FOR
#    antamil     07/02/25  - Bug 38135681 Dbserver patch files cleanup for
#                            single vm patching for EXACC
#    antamil     06/25/25  - Bug 38089462 Dbserver patch files cleanup for single vm
...
#    remamid     06/11/25 - Check for patch file existence before performing
#                           space calculation bug 37916448
#    araghave    06/05/25 - Enh 38039225 - RE-RUN PATCHMGR VERIFY CONFIG IN
#                           CASE OF A FAILURE DUE TO SWITCH MISCONFIGURATION
#                           DURING PROVISIONING
#    avimonda    06/03/25 - Enh 37849958 - AUTO CLEANUP OF
#                           DBSERVER.PATCH.ZIP_EXADATA_OL8_*_LINUX-X86-64.ZIP
#                           AFTER OS PATCHING
#    remamid     05/22/25 - Remove dbmcli execution for DomU bug 37878442
...
#                           PATCH COMBINATIONS
#    jyotdas     04/28/25 - Bug 37564769 - Exit immediately if patching wf has
#                           already failed during retry after ecra upgrade
...
#    apotluri    04/02/25 - Bug 37780427 - CONCURRENT EXECUTION OF
#                           DBSERVER_BACKUP.SH DURING CELL ROLLBACK NON ROLLING
#                           IS CAUSING BACKUP PARTITION TO BE EMPTY
...
#    apotluri    02/18/25 - Enhancement Request 37590732 - INFRAPATCHING CODE
...
#                           SKIPPED
#    nelango     02/17/25 - Bug 37328906: ipmi servicestate checks during
#                           precheck
...
#    diguma      01/22/25 - bug 37492318: INFRAPATCHING LAYER INTERFERING WITH
#                           THE BLACKOUTS SET BY FPMS/FPE ON ADBD DOMU NODES
#    emekala     01/20/25 - BUG 37414318 - EXACC PATCHING ON FEDRAMP REGION IS
#                           NOT CHECKING FOR SELINUX ENFORCING
...
#    bhpati      12/03/24 - Enh 37162258 - EXACC GEN2 | PATCHING | DIAG TAR
...
#    avimonda    11/25/24 - Enhancement Request 37232972 - OPTIMIZE CONNECTION
#                           TIMEOUT TO REDUCE FAILED PRE-CHECK TIME
#    araghave    11/22/24 - Enh 37270708 - MAKE RELEVANT CODE CHANGES AND TEST
#                           AN IBSWITCH/ADMIN SWITCH COMBINATION
...
class TargetHandler(GenericHandler):
    ...
    def __init__(self, *initial_data, **kwargs):
        ...
        super(TargetHandler, self).__init__(*initial_data, **kwargs)
        self.mPatchLogInfo("TargetHandler")
        ...
        # indicates if blackout should be set on this session
        self._disable_blackout = False
        ...
        # dbnu plugin changes must be called only during dom0/domu patching.
        if self.mGetTargetTypes()[0] not in [ PATCH_CELL, PATCH_IBSWITCH, PATCH_SWITCH, PATCH_ROCESWITCH ]:
        ...
        if _sshEnvDict:
            ...
            for _index in range(len(_sshEnvDict["fromHost"])):
                ...
                try:
                    ...
                    if aSingleVmHandler:
                        aSingleVmHandler.mCleanupSSHForSingleVM(_sshEnvDict["fromHost"][_index], _sshEnvDict["remoteHostLists"][_index], \
                                               _ssh_env, self)
                    else:
                        _ssh_env.mCleanSSHPasswordless(_sshEnvDict["fromHost"][_index],
                        ...
                        _ssh_env.mAddToKnownHosts(_sshEnvDict["fromHost"][_index], _sshEnvDict["remoteHostLists"][_index])
                        '''
                        Below checks are applicable to Dom0 and Domu targets
    ...
    def mSetDom0ToPatchcellSwitches(self):
        ...
        self.__dom0_to_patch_cells_switches = self.mSetLaunchNodeAsPatchBase(
                ...
                aPatchZipSizeMb=self.mGetCellSwitchesPatchZipSizeMb(),
    ...
    def mCheckAndUpdateNodeList(self, aNodeList):

        _reachableNodeList  = []
        for _node_name in aNodeList:
            _node = exaBoxNode(get_gcontext())
            if _node.mIsConnectable(aHost=_node_name, aTimeout=mGetSshTimeout(), aKeyOnly=True):
                _reachableNodeList.append(_node_name)
            else:
                self.mPatchLogError(f"Unable to connect to node [{str(_node_name)}].")

        return _reachableNodeList
    ...
    def mGetSwitchTargetVersion(self):
        """
        ...
        if self.mGetCurrentTargetType() in [PATCH_IBSWITCH]:
        ...
        elif self.mGetCurrentTargetType() in [ PATCH_ROCESWITCH ]:
            ...
            self.mPatchLogError(
                f"Invalid Target Type : {self.mGetCurrentTargetType()}, Target can either be IBSwitches or RoceSwitches.")
    ...
    def mGetRoceIbSwitchTargetVersionToUpdateImageDetails(self):
        """
        Gets the ibswitch,roceswitch firmware target version. EXADATA_IMAGE_IBSWITCH_UPGRADE_VERSION,
        EXADATA_IMAGE_ROCESWITCH_UPGRADE_VERSION and are env variables used in the patchmgr to set the
        switch version.

        This function looks for these variables using printenv. If not found, then it takes the default
        value being used in patchmgr script and the return value will be used to populate the imageversion
        details in the status reporting output.
        """
    ...
    def mSetcellSwitchesBaseEnvironment(self):
        ...
        if self.mGetCellSwitchesLocalPatchZip():
            ...
            self.__cells_switches_patch_necessary_space_mb = int((self.mGetCellSwitchesPatchZipSizeMb() * 2 +
    ...
    def mParsePatchmgrXml(self, patchmgrxml, aFinalLastCns):
        ...
        _cnsjson["data"]["oeda_requests_log_path"] = _oeda_request_log_path
        ...
        if self.mGetAdditionalOptions() and ('LaunchNode' in self.mGetAdditionalOptions()[0]):
            _cnsjson["data"]["LaunchNode"] = self.mGetAdditionalOptions()[0]['LaunchNode']
        ...
        if self.mIsClusterLessUpgrade():
            ...
            if self.mGetAdditionalOptions() and ('LaunchNodeCabinet' in self.mGetAdditionalOptions()[0]):
                _cnsjson["data"]["LaunchNodeCabinet"] = self.mGetAdditionalOptions()[0]['LaunchNodeCabinet']
            if self.mGetAdditionalOptions() and ('CabinetFabricName' in self.mGetAdditionalOptions()[0]):
                _cnsjson["data"]["CabinetFabricName"] = self.mGetAdditionalOptions()[0]['CabinetFabricName']
            if self.mGetAdditionalOptions() and ('LaunchNodeState' in self.mGetAdditionalOptions()[0]):
                _cnsjson["data"]["LaunchNodeState"] = self.mGetAdditionalOptions()[0]['LaunchNodeState']
        else:
        ...
        elif self.mGetCurrentTargetType() in [ PATCH_IBSWITCH, PATCH_ROCESWITCH ]:
            if self.mGetCurrentTargetType() == PATCH_IBSWITCH:
                _node_type = 'IBSwitch'
            elif self.mGetCurrentTargetType() == PATCH_ROCESWITCH:
                _node_type = 'ROCESwitch'
            _channel_info["topicId"] = "critical.patch_of_exadata_infrastructure.patch_Exadata_switch"
            _cnsjson["data"]["topic"] = "critical.patch_of_exadata_infrastructure.patch_Exadata_switch"
    ...
    def mSetTopicForCns(self, channel_info, cnsjson, target_succ_flag, target_na_flag):
        ...
        elif target_succ_flag and self.mGetCurrentTargetType() in [ PATCH_IBSWITCH, PATCH_ROCESWITCH ]:
            channel_info['topicId'] = "critical.patch_of_exadata_infrastructure.patch_Exadata_switch.SUCCESS"
            cnsjson['data']['topic'] = "critical.patch_of_exadata_infrastructure.patch_Exadata_switch.SUCCESS"
        ...
        elif target_na_flag and self.mGetCurrentTargetType() in [ PATCH_IBSWITCH, PATCH_ROCESWITCH ]:
            channel_info['topicId'] = "critical.patch_of_exadata_infrastructure.patch_Exadata_switch.NOTAPPLICABLE"
            cnsjson['data']['topic'] = "critical.patch_of_exadata_infrastructure.patch_Exadata_switch.NOTAPPLICABLE"
        ...
        # verify and set Fail status
            ...
            channel_info['topicId'] = "critical.patch_of_exadata_infrastructure.patch_Exadata_switch.FAIL"
            cnsjson['data']['topic'] = "critical.patch_of_exadata_infrastructure.patch_Exadata_switch.FAIL"
    ...
    def mParseCellPatchmgrXml(self, patchmgrxml):
        #Parse  patchmgr xml file  to find failed cell nodes
        _patch_failure_node = []
        ...
        try:
            ...
            for _each_node_type in _root.findall('./' + _node_type):
                _individual_node_data = {"node_name": _each_node_type.attrib['NAME']}

                # Processing last patch transition state
                _cur_pstate_tran = None
                for _each_pstate_tran in _each_node_type.findall('./Patch_State/Transition'):
                    _cur_pstate_tran = _each_pstate_tran
                ...
                if _cur_pstate_tran is not None:
                    ...
                    if _individual_node_data["patchmgr_status"] == "Failed":
                        _patch_failure_node.append(_individual_node_data["node_name"].split('.')[0])
    ...
    def mCleanEnvironment(self, aDom0, aNodesList, aListFilePath, aBaseDir, aLogDir, aNodeType, aPatchExitStatus):
        ...
        if aNodeType in [ PATCH_IBSWITCH, PATCH_ROCESWITCH, PATCH_CELL, PATCH_ADMINSWITCH ]:
        ...
        # Get RoceSwitch/Admin Switch specific logs
    ...
    def mGetPatchMgrMiscLogFiles(self, aDom0, aRemotePath, aTaskType=None,aNodesList=[]):
        ...
        '''
        if mExaspliceVersionPatternMatch(self.mGetTargetVersion()) and \
    ...
    def mCheckFreshInstall(self, aDom0):
        ...
        _node = exaBoxNode(get_gcontext())
        _node.mConnect(aHost=aDom0)
        _i, _o, _e = _node.mExecuteCmd(_cmd_imagehistory_cmd, aTimeout=SHELL_CMD_DEFAULT_TIMEOUT_IN_SECONDS)
        mCheckAndFailOnCmdTimeout(aCmd=_cmd_imagehistory_cmd, aNode=_node, aHandler=self)
        _out = _o.readlines()
        if _out[0] == 1:
            _i, _o, _e = _node.mExecuteCmd(_cmd_get_fresh_image_version, aTimeout=SHELL_CMD_DEFAULT_TIMEOUT_IN_SECONDS)
            mCheckAndFailOnCmdTimeout(aCmd=_cmd_get_fresh_image_version, aNode=_node, aHandler=self)
    ...
    def mPrepareEnvironment(self, aDom0, aNodesList, aBaseDir, aAdminSwitch=False):
        ...
        if self.__ssh_env_setup_switches_cell:
            '''
             If targettype is present in _get_exec_key_scan_on_target_node list , the execute 
             ssh-keyscan on the targetnode and fetch the data , since ssh-keyscan is not working 
             on dom0 launch node for switch
            '''
            _get_exec_key_scan_on_target_node = self.mGetKeyScanFromTargetTypeNodeList()
            if len(_get_exec_key_scan_on_target_node) > 0 and (self.mGetCurrentTargetType().lower()) in _get_exec_key_scan_on_target_node:
                _key = self.__ssh_env_setup_switches_cell.mSetSSHPasswordlessForOlVersionMismatch(aDom0, aNodesList, self.mGetCurrentTargetType(), False)
            elif self.mGetCurrentTargetType() == PATCH_ROCESWITCH or aAdminSwitch:
                _key = self.__ssh_env_setup_switches_cell.mSetSSHPasswordless(aDom0, aNodesList, True, aKeyComment=EXAPATCHING_KEY_TAG)
            else:
                _key = self.__ssh_env_setup_switches_cell.mSetSSHPasswordless(aDom0, aNodesList, aKeyComment=EXAPATCHING_KEY_TAG)
        ...
        '''
        self.mPatchLogInfo(f"Passwordless ssh validation performed between {str(aNodesList)} and {aDom0}")
        self.mGetCluPatchCheck().mVerifyPatchmgrSshConnectivityBetweenExadataHosts(aNodesList, aDom0, aAdminSwitch=aAdminSwitch)
            ...
            if aConnectibleDomUList and len(aConnectibleDomUList) > 0:
                ...
                try:
                    ...
                    def _mValidateRDSPing(aNode, aStatus):
                        ...
                        try:
                            if self.mGetCurrentTargetType() == PATCH_DOM0:
                                _user_to_connect_with = self.mGetUsertoConnectWith(aNode, _node)
    ...
    def mUpdateCurrentLaunchNodeDetailsInCorrespondingTaskHandlerInstance(self, aNode=None, aPatchmgrLogPathLaunchNode=None):
        ...
        try:
            ...
            self.mPatchLogError(f"Exception {str(e)} occurred in mUpdateCurrentLaunchNodeDetailsInCorrespondingTaskHandlerInstance")
            self.mPatchLogTrace(traceback.format_exc())
    ...
    def mGetKnownAlertHistoryCmd(self, aTargetType, aCurrentImageVersion):
        ...
        if aTargetType in [ PATCH_DOM0 ]:
            ...
            if (version_compare(_currentImageVersion, "25")) >= 1:
                _dbmcli_cmd = "dbmcli --inline -e 'LIST ALERTHISTORY WHERE endtime=null AND alerttype=stateful'"
            else:
            ...
            self.mPatchLogInfo("Skipping the check for critical hardware alerts on DomU, as it was only searching for the 'Exadata cloud services test message' pattern which will never appear in DomU anyway")
        elif aTargetType in [ PATCH_CELL ]:
            _cellcli_cmd =''
            if (version_compare(_currentImageVersion, "25")) >= 1:
                _cellcli_cmd = "cellcli --inline -e 'LIST ALERTHISTORY WHERE endtime=null AND alerttype=stateful'"
            else:
    ...
    def mCheckKnownAlertHistory(self, aTargetType, aListOfNodes):
        ...
        if aTargetType in [ PATCH_DOMU ]:
            self.mPatchLogInfo("Skipping known alerthistory checks for DomU patching.")
            return True
        ...
        for aNode in aListOfNodes:
            ...
            _cmd = self.mGetKnownAlertHistoryCmd(aTargetType, _currentImageVersion)
            if _cmd == '' :
                _ignore_alert_flag |= True
    ...
    def mGetHardwareCriticalAlertHistoryCmd(self, aTargetType, aCurrentImageVersion):
        ...
        if aTargetType in [ PATCH_DOM0 ]:
            ...
            if (version_compare(_currentImageVersion, "25")) >= 1:
              _dbmcli_cmd = "dbmcli --inline -e 'LIST ALERTHISTORY WHERE endtime=null AND alerttype=stateful and alertShortName=Hardware and severity=Critical'"
            else:
            ...
            if (version_compare(_currentImageVersion, "25")) >= 1:
                _cellcli_cmd = "cellcli --inline -e 'LIST ALERTHISTORY WHERE endtime=null AND alerttype=stateful and alertShortName=Hardware and severity=Critical'"
            else:
    ...
    def mCheckHwCriticalAlert(self, aTargetType, aListOfNodes):
        ...
        for aNode in aListOfNodes:
            ...
            _cmd = self.mGetHardwareCriticalAlertHistoryCmd(aTargetType, _currentImageVersion)
            if _cmd == '' :
        ...
        if aNodeList:
            ...
            if ret != PATCH_SUCCESS_EXIT_CODE:
                if aUpdateExacloudDB:
                    ...
                    self.mPatchLogInfo("Exacloud DB not updated with status code related to existence of patchmgr/dbnodeupdate.sh sessions in the cluster.")
    ...
    def mVerifyAndCleanupMissingPatchmgrRemotePatchBase(self, aRemotePatchBase, aLaunchNodeCandidates):
        ...
        for _launch_node in aLaunchNodeCandidates:
            try:
                ...
                _errmsg = f"Unable to verify and cleanup missing patchmgr remote patch base on Node : [{_launch_node}]. Error : {str(e)}"
                _ret = MISSING_REMOTE_PATCH_BASE_CLEANUP_FAILED
                self.mAddError(_ret, _errmsg)
                self.mPatchLogWarn(_errmsg)
                self.mPatchLogTrace(traceback.format_exc())
                raise Exception(_errmsg)
    ...
    def mSetLaunchNodeAsPatchBase(self,
        ...
        # Exadata patch cleanup is skipped if launch node is localhost
        if self.mGetInfrapatchExecutionValidator().mCheckCondition('mIsLocalHostLaunchNode'):
            ...
            for _cluster, _domu_list_for_one_cluster in _clusterToVMList:
                if _cluster == self.mGetRackName():
                    _domU_list = _domu_list_for_one_cluster
                    break
            if _domU_list:
                _ret = aSingleVmHandler.mCleanupExadataPatchesSingleVMForExaCS(self, aLaunchNodeCandidates[0], _domU_list[0], aPatchBaseDir)
                if _ret != PATCH_SUCCESS_EXIT_CODE:
                    self.mPatchLogWarn(f"Exadata patches not cleaned properly.")
            else:
                self.mPatchLogWarn("Patch cleanup was not performed")
        else:
            # Cleanup old patches before validating for space requirements
            # and copying patches.
            _ret_stats = self.mCleanupExadataPatches(aLaunchNodeCandidates)
            for _ret, _errmsg in _ret_stats.items(): 
                if _ret != PATCH_SUCCESS_EXIT_CODE:
                    self.mPatchLogWarn(f"Exadata patches not cleaned properly. Error: {_ret}, Message: {str(_errmsg)}")
                    if _ret == PATCHING_CONNECT_FAILED:
                        self.mAddError(_ret, _errmsg)
                        raise Exception(_errmsg)
        ...
        _ret, _errmsg = self.mValidateImageCheckSumWithRetry(aPatchZipName, _local_patch_path, aRemotePatchBase ,aLaunchNodeCandidates , aRemoteNecessarySpaceMb, aSingleVmHandler)
        if _ret != PATCH_SUCCESS_EXIT_CODE:
            raise Exception(_errmsg)
        ...
        if aMoreFilesToCopy:
            for _file, _copy_to in aMoreFilesToCopy:
                ...
                _ret, _errmsg = self.mValidateImageCheckSumWithRetry(_file_name, _local_patch_path, _copy_to ,aLaunchNodeCandidates, aRemoteNecessarySpaceMb,aSingleVmHandler)
                if _ret != PATCH_SUCCESS_EXIT_CODE:
                    raise Exception(_errmsg)
    ...
    def mValidateImageCheckSumWithRetry(self,  aPatchFile, aPatchRepo, aRemotePatchBase, aNodeList, aRemoteNecessarySpaceMb, aSingleVmHandler=None):
        """
        ...
        while _retries < _max_retries:
            try:
                if self.mGetInfrapatchExecutionValidator().mCheckCondition('mIsCpsLaunchNodeForDomU'):
                    ...
                    _rc, _errmsg = self.mValidateImageCheckSum(aPatchFile, aPatchRepo, aRemotePatchBase, aNodeList, aRemoteNecessarySpaceMb, aSingleVmHandler)
    ...
    def mValidateImageCheckSum(self, aPatchFile, aPatchRepo, aRemotePatchBase, aNodeList, aRemoteNecessarySpaceMb, aSingleVmHandler):
        """
        ...
        def _mCopyFile(aStatus):
            ...
            try:
                ...
                if aSingleVmHandler:
                    aSingleVmHandler.mCopyDbserverPatchFile(_remote_node, _local_patch_file, _remote_patch_file, self, _node)
                else:
    ...
    def mUpdatePatchProgressStatus(self, aNodeList=[], aDiscardedNodeList=[], aPatchmgrXmlData=None, aSleepStatus=None, aNodeType=None):
        ...
        _patch_start_time_patch_transition_state_map = {
            ...
            "adminswitch-rolling-patch": "Updating",
            "adminswitch-rolling-rollback": "Downgrading",
            "adminswitch-rolling-patch_prereq_check": "Started",
            "adminswitch-rolling-rollback_prereq_check": "Started",
            "adminswitch-non-rolling-patch": "Updating",
            "adminswitch-non-rolling-rollback": "Downgrading",
            "adminswitch-non-rolling-patch_prereq_check": "Started",
            "adminswitch-non-rolling-rollback_prereq_check": "Started"
        }
        ...
        if "node_patching_progress_data" not in _patch_progressing_status_json:
            ...
            if _target_type == PATCH_DOMU:
                ...
                if self.mIsAdminSwitchPatchingEnabled(): 
                    _node_list += self.mGetListOfAdminSwitches()
    ...
    def mCleanupExadataPatches(self, aLaunchNodeCandidates):
        ...
        _cleanup_failed_node_list = []
        _connect_failed_node_list = []
        _error_status = {}
        _return_status = {PATCH_SUCCESS_EXIT_CODE: None}
        ...
        def _purge_exadata_patches(aNode, aStatus):
            ...
            _list_of_files_not_to_be_purged_string = None
            _purge_patches = True
            _error_str = ''
            try:
                _node.mConnect(aHost=aNode)
                ...
                if _active_image_version and _inactive_image_version and self.mGetTargetVersion():

                    self.mPatchLogInfo(f"Active Exadata image version : {str(_active_image_version)}")
                    self.mPatchLogInfo(f"Inactive Exadata image version : {str(_inactive_image_version)}")
                    self.mPatchLogInfo(f"Target Exadata image version : {str(self.mGetTargetVersion())}")

                    _filtered_versions_to_keep =[]
                    _keep_versions = [_active_image_version, _inactive_image_version, self.mGetTargetVersion()]

                    for _version in _keep_versions:
                        if 'exasplice' in _version:
                            _filtered_versions_to_keep.append(_version.split(".exasplice.", 1)[0])
                        else:
                            _filtered_versions_to_keep.append(_version)

                    self.mPatchLogInfo(f"Filtered Active Exadata image version : {str(_filtered_versions_to_keep[0])}")
                    self.mPatchLogInfo(f"Filtered Inactive Exadata image version : {str(_filtered_versions_to_keep[1])}")
                    self.mPatchLogInfo(f"Filtered Target Exadata image version : {str(_filtered_versions_to_keep[2])}")

                    _list_of_files_not_to_be_purged_string = '|'.join(_filtered_versions_to_keep)
                ...
                if _purge_patches:
                    ...
                    if _list_of_files_not_to_be_purged_string:
                        _cmd_list_files_patch_to_be_purged = f"ls -ld {_remote_patch_base}/*patch.zip* | egrep -v '{_list_of_files_not_to_be_purged_string}' | /usr/bin/awk '{{print $9}}' | xargs rm -rfv"
                        _node.mExecuteCmdLog(_cmd_list_files_patch_to_be_purged)

            except Exception as e:
                self.mPatchLogWarn(f"Unable to purge exadata patches on Node : [{aNode}]. Error : {str(e)}")
                self.mPatchLogTrace(traceback.format_exc())
                if _node.mIsConnected():
                    aStatus.append({'node': aNode, 'status': 'failed'})
                else:
                ...
                if _node.mIsConnected():
                    ...
                    _ret = PATCHING_CONNECT_FAILED
                    aStatus.append({'node': aNode, 'status': 'failed', 'errorcode':_ret, 'errorstr': _error_str})
    ...
    def mValidateRootFsSpaceUsage(self, aNodeList=None):
        """
        ...
        if self.mGetInfrapatchExecutionValidator().mCheckCondition('mIsSpaceValidationEnabledForTarget'):
            # aNodeList contains only the nodes which are to be patched..Already upgraded nodes are not considered here
            if aNodeList:
                _node_list = aNodeList
                if self.mGetCurrentTargetType() == PATCH_DOM0:
    ...
    #     }
        ...
        #If patchmgr has already failed before ECRA upgrade , no need to continue
        if _idempotency_data and 'errorCode' in _idempotency_data.keys():
            _error_code = _idempotency_data["errorCode"]
            if "errorMessage" in _idempotency_data.keys():
                _error_message = _idempotency_data["errorMessage"]
                if _error_message:
                    self.mPatchLogInfo(f"Error Message from Idempotency Data is {_error_message}")
            if _error_code and _error_code != PATCH_SUCCESS_EXIT_CODE:
                _ret = _error_code
                _suggestion_msg = f"Retry ECRA Request ID {self.mGetMasterReqId()} has already failed before ECRA Upgrade with  error code {_error_code}. Failing infrapatching on Retry after ECRA upgrade"
        ...
        if _patch_mgr_log_dir and _launch_node:
            ...
            _p_ses_exist, _p_active_node = self.mCheckPatchmgrSessionExistence(_patch_mgr_log_dir,aLaunchNode=_launch_node,aNodeList=None, aUpdateExacloudDB = False)
        ...
        if _p_ses_exist != PATCHMGR_SESSION_ALREADY_EXIST:
            ...
            _reachableNodeList = self.mCheckAndUpdateNodeList(_launch_node_list)
            for _ln in _reachableNodeList:
    ...
    def mValidateServiceStateOnIlom(self, aNodeList):
        _ret = PATCH_SUCCESS_EXIT_CODE
        _val_failed_nodes = []
        _node = exaBoxNode(get_gcontext())
        ...
        if not self.mGetInfrapatchExecutionValidator().mCheckCondition('enableServiceStateOnIlomsPriorToDom0CellPatchingEnabled'):
            return _ret , _val_failed_nodes

        for _node_name in aNodeList:
            try:
                _node.mConnect(aHost=_node_name, aTimeout=30)
                ...
                if _node.mGetCmdExitStatus() == 0:
                    if _out:
                        ...
                        if _output:
                            self.mPatchLogInfo(
                                f"Service State of ilom on node : {_node_name}. output : {str(_output)}.")
                            if _output[0].find("disabled") != -1:
                                self.mPatchLogWarn(
                                    f"Service State of ilom on node : {_node_name} is disabled. Not attempted to enable it in precheck")
                        else:
                            self.mPatchLogError(
                                f"ipmitool has returned empty list for Service State of ilom on node : {_node_name}.")
                            _ret = INFRA_PATCHING_IPMI_SERVICESTATE_READ_FAILED
                            _val_failed_nodes.append(_node_name)
                    else:
                        self.mPatchLogError(
                            f"Service State of ilom on node : {_node_name} is empty.")
                        _ret = INFRA_PATCHING_IPMI_SERVICESTATE_READ_FAILED
                        _val_failed_nodes.append(_node_name)
                else:    
                    self.mPatchLogError(
                        f"Error in reading ipmi service state value of ilom on node : {_node_name}.")
                    _ret = INFRA_PATCHING_IPMI_SERVICESTATE_READ_FAILED
                    _val_failed_nodes.append(_node_name)
                    
            except Exception as e:
                self.mPatchLogError(
                    f'Exception in validating servicestate value on ilom of node : {_node_name} Error : {str(e)}')
                self.mPatchLogTrace(traceback.format_exc())
                _ret = INFRA_PATCHING_IPMI_SERVICESTATE_READ_FAILED
                _val_failed_nodes.append(_node_name)
            finally:
            ...
            try:
                ...
                if aPhase == "prepatch":
                        ...
                        if _o:
                            ...
                            if _o[0].find("disabled") != -1:
                                ...
                                if _node.mGetCmdExitStatus() == 0:
                                    ...
                                    _ret = INFRA_PATCHING_IPMI_SERVICESTATE_NOT_ENABLED
                                    _val_failed_nodes.append(_node_name)
                        else:
                            self.mPatchLogError(
                                f"ipmitool has returned empty list for Service State of ilom on node : {_node_name}.")
                    ...
                    if _out:
                        ...
                        if _o:
                            self.mPatchLogInfo(
                                f"Service State of ilom on node : {_node_name}  post patching, output : {str(_o)}.")
    ...
    def mSetLaunchNodeAsPatchBaseForLocalNode(self,
        ...
        for _cluster, _domu_list_for_one_cluster in _clusterToVMList:
            if _cluster == self.mGetRackName():
                _domU_list = _domu_list_for_one_cluster
                break
        if _domU_list and aSingleVmHandler:
            _ret = aSingleVmHandler.mCleanupExadataPatchesSingleVMForExaCC(self, _domU_list[0], aPatchBaseDir)
            if _ret != PATCH_SUCCESS_EXIT_CODE:
                self.mPatchLogWarn(f"Exadata patches not cleaned properly.")
            else:
                self.mPatchLogWarn(f"Exadata patches cleaned properly.")
        else:
            self.mPatchLogWarn(f"Exadata patches not cleaned.")
    ...
    def mGetLocalNodePatchMgrMiscLogFiles(self, aRemotePath, aTaskType=None, aNodesList=[]):
        ...
        '''
         ...
         2021-01-19 13:31:08+0000 - Dom0Handler - INFO - - patchmgr.log
        '''
        if mExaspliceVersionPatternMatch(self.mGetTargetVersion()) and aTaskType in [ TASK_PATCH, TASK_PREREQ_CHECK ]: 
    ...
    def mSetConnectionUser(self, _node):
        ...
        if self.mGetInfrapatchExecutionValidator().mCheckCondition('mIsManagementHostLaunchNodeForDomuOrClusterless'):
            _user = mGetLaunchNodeConfig('MANAGEMENT_HOST', 'user')
            if _node.mIsConnected():
        ...
        try:
            ...
            if _node.mFileExists(_tfa_bin):
                if aEnable:
                    ...
                    _in, _out, _err = _node.mExecuteCmd(f"{_tfa_bin} blackout print | grep HOST|grep ACTIVE")
                    if _node.mGetCmdExitStatus() == 0:
                        # if blackout is already set, skip
                        self._disable_blackout = False 
                        self.mPatchLogInfo(f"Blackout is already set on {aNode}")
                    else:
                        self.mPatchLogInfo(f"Setting blackout on {aNode}")
                        _reason = "DomU VM OS Patching " + aNode
                        _node.mExecuteCmdLog(f"{_tfa_bin} blackout add -targettype host -timeout 2h -reason '{_reason}'")
                        _node.mExecuteCmdLog(f"{_tfa_bin} blackout print")
                        self._disable_blackout = True
                elif self._disable_blackout:
    ...
    def mExecuteAdminSwitchInfraPatchOperations(self):
        '''
         This method takes care of performing all admin
         switch patch operations both as part of regular
         and idempotency flow.
        '''
        _ret = PATCH_SUCCESS_EXIT_CODE
        _no_action_taken =0
        ...
        try:
            self.mPatchLogInfo(
                f"\n\n---------------> Starting {self.mGetTask()} in {PATCH_ADMINSWITCH} <---------------\n\n")

            # Check for idempotency
            if self.mPatchRequestRetried():
                _ret  = self.mCheckAdminSwitchIdemPotency()
            else:
                _ret, _no_action_taken = self.mAdminSwitchRegularPatchRun()

            self.mPatchLogInfo(f"Task: {_task_type} - Type: {PATCH_ADMINSWITCH}\t\t[ ret_code = {_ret} ]")
            if _ret != PATCH_SUCCESS_EXIT_CODE:
                self.mPatchLogError(f"{PATCH_ADMINSWITCH} '{_task_type}' failed.")
            ...
            if _child_request_error_already_exists_in_db:
                ...
                if self.mGetTask() in [ TASK_PREREQ_CHECK, TASK_ROLLBACK_PREREQ_CHECK ]:
                    _ret = SWITCH_PATCH_PRECHECK_FAILED
                elif self.mGetTask() == TASK_PATCH:
                    _ret = SWITCH_PATCH_FAILED
                elif self.mGetTask() == TASK_ROLLBACK:
                    _ret = SWITCH_ROLLBACK_FAILED
                _suggestion_msg = f"{PATCH_ADMINSWITCH} '{_task_type}' failed."
                self.mAddError(_ret, _suggestion_msg)
    ...
    def mCheckAdminSwitchIdemPotency(self):
        '''
         This method takes care of performing admin switch
         patch operations as part of idempotency flow.
        '''
        ...
        _patchMgrObj = InfraPatchManager(aTarget=PATCH_ADMINSWITCH, aOperation=self.mGetTask(),
                                         aPatchBaseAfterUnzip=self.mGetCellSwitchesPatchBaseAfterUnzip(),
        ...
        # Read list of Adminswitch from input file
        _list_of_adminswitches_retry_case = _patchMgrObj.mGetNodeListFromNodesToBePatchedFile(
            aHost=self.mGetDom0ToPatchcellSwitches())
    
        # Clean the environment: Delete passwordless, delete input file
        self.mCleanEnvironment(self.mGetDom0ToPatchcellSwitches(), _list_of_adminswitches_retry_case,
                               _input_file_retry_case, self.mGetCellSwitchesPatchBaseAfterUnzip(),
                               self.mGetPatchmgrLogPathOnLaunchNode(), PATCH_ADMINSWITCH, _ret)
    
        return _ret

    def mAdminSwitchRegularPatchRun(self):
        '''
         This method takes care of performing admin switch
         patch operations as part of Regular patch flow.
        '''
        _no_action_taken = 0
        _ret = PATCH_SUCCESS_EXIT_CODE
        _discarded = []
        ...
        if len(self.mGetListOfAdminSwitches()) > 0:
            ...
            _key = self.mPrepareEnvironment(self.mGetDom0ToPatchcellSwitches(), self.mGetListOfAdminSwitches(),
                                                           self.mGetCellSwitchesPatchBaseAfterUnzip(), aAdminSwitch=True)

            '''
             Since there is a possibility of admin and roce/ibswitches patched
             together, log details get overwritten, we are appending a tag "_admin_switch"
             for the patchmgr log path.
            '''
            self.mSetPatchmgrLogPathOnLaunchNode(self.mGetCellSwitchesPatchBaseAfterUnzip() + "patchmgr_log_" + self.mGetMasterReqId() + "_admin_switch")
            ...
            _input_file = _patchMgrObj.mCreateNodesToBePatchedFile(aLaunchNode=self.mGetDom0ToPatchcellSwitches(),
                                                                   aHostList=self.mGetListOfAdminSwitches())

            # Run patchmgr command
            _ret = self.mPatchAdminswitchesRolling(self.mGetListOfAdminSwitches(), _patchMgrObj)

            # Clean the environment: Delete passwordless, delete input file
            self.mCleanEnvironment(self.mGetDom0ToPatchcellSwitches(), self.mGetListOfAdminSwitches(), _input_file,
            ...
            # status to previous
            _suggestion_msg = "No available Adminswitches to run the patchmgr. Nothing to do here."
            _ret = PATCH_SUCCESS_EXIT_CODE
            self.mAddError(_ret, _suggestion_msg)
    ...
    def mPatchAdminswitchesRolling(self, aListOfNodes, aPatchMgrObj):
        """
        Runs the Adminswitches patch operations in rolling mode.
        """
        _exit_code = PATCH_SUCCESS_EXIT_CODE
        _patchmgr_cmd = ""
        ...
        if self.mGetTask() in [TASK_PATCH, TASK_ROLLBACK]:
            ...
            _patchMgrObj.mWaitForPatchMgrCmdExecutionToComplete(aInputListFile=_input_file,
                                                                aPatchStates=STEP_RUN_PATCH_SWITCH)
        else:
            _patchMgrObj.mWaitForPatchMgrCmdExecutionToComplete()
        self.mPatchLogInfo(
            "Finished waiting for Patch Manager command execution. Starting to handle exit code from Patch Manager")
    ...
    def mPrintPatchSwitchTypePrintActions(self):
        '''
         Based on the PatchSwitchType option passed,
         Info messages related to the action taken will
         be printed.
        '''
        _patch_switch_type_messages = {
            'all': "Specified PatchSwitchType option is 'all'. All Roce leaf,spine(if any), admin switches will be patched. All IBSwitches will be patched in case of a Xen environment.",
            'none': "Specified PatchSwitchType option is 'none'. Switch patching is skipped in this case.",
            'roce': "Specified PatchSwitchType option is 'roce'. Only Roce leaf switches on KVM and all IBSwitches on Xen environments will be patched. Admin and Spine switch patching is skipped.",
            'rocespine': "Specified PatchSwitchType option is 'rocespine'. Both Roce leaf and spine switches on KVM and all IBSwitches on Xen environments will be patched. Admin switch patching is skipped.",
            'spine': "Specified PatchSwitchType option is 'spine'. Only Roce spine switches on EXACC environments will be patched. Admin and leaf switch patching is skipped.",
            'admin': "Specified PatchSwitchType option is 'admin'. Only admin switches will be patched both on EXACC Xen and KVM environments."
        }

        if self.mIsExaCC() and self.mGetPatchSwitchType()  in _patch_switch_type_messages:
            self.mPatchLogInfo(_patch_switch_type_messages[self.mGetPatchSwitchType()])
            self.mPatchLogInfo("Please note that the PatchSwitchType can only be configured through ecra property and cannot be specified through patch commands.")
        else:
            self.mPatchLogInfo("Environment is EXACS and PatchSwitchType option will not be specified. Admin and Roce Switch patching not supported on EXACS. Only IBSwitch patching supported.")
           
    def mIsAdminSwitchPatchingEnabled(self):
        '''
         Admin switch patching will be enabled only
         when PatchSwitchType is set to admin or all.
        '''
        if (self.mGetPatchSwitchType().lower() in [ "all", "admin" ]):
            return True
        else:
            return False
            
    def mIsSwitchPatchingSkipped(self):
        '''
         Switch patching will be skipped in case
         of PatchSwitchType is passed as none or null 
         or empty.
        '''
        if (self.mGetPatchSwitchType().lower() in [ "none", "null", "" ]):
            return True
        else:
            return False
