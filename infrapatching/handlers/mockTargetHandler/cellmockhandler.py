#
# $Header: ecs/exacloud/exabox/infrapatching/handlers/mockTargetHandler/cellmockhandler.py /main/5 2024/12/24 20:14:02 diguma Exp $
#
# cellmockhandler.py
#
...
#
#    NAME
#      cellmockhandler.py - Patch - Cell Basic Functionality
#
#    DESCRIPTION
...
from exabox.infrapatching.utils.utility import mRegisterInfraPatchingHandlers, mReadCallback, mErrorCallback, mGetSshTimeout
...
class CellMockHandler(TargetMockHandler):

    def __init__(self, *initial_data, **kwargs):

        super(CellMockHandler, self).__init__(*initial_data, **kwargs)
        self.mPatchLogInfo("CellMockHandler")
    ...
    def mVMOperation(self, aAction):
        ...
        _process = None
        '''
         All of the Dom0s need to be considered for shutdown.
        '''
        for _dom0 in self.mGetCustomizedDom0List():
            if aAction == 'shutdown':
                self.mUpdatePatchStatus(True, STEP_SHUTDOWN_VMS, _dom0)
                '''
                 All of the DomUs need to be considered for shutdown.
                '''
                self.__dom0s_list[_dom0] = self.mGetDomUListFromXml(_dom0, aFromXmList=True)
                self.mPatchLogInfo(f"Dom0 = {_dom0}, DomUs within dom0 = {self.__dom0s_list[_dom0]}")

                """
                If the graceful shutdown of the VM does not happen, xm destroy/virsh destroy would be called.                
                """
                _process = ProcessStructure(self.mGetCluPatchCheck().mShutDownVMs,
                                            [_dom0, self.__dom0s_list[_dom0], _rc_status], _dom0)
            elif aAction == 'start':
                ...
                self.mPatchLogError(f"Invalid VM operation specified = {aAction}")
                return False
    ...
    def mRegularPatchRun(self):
        ...
        if not self.mIsMockEnv() and self.mGetTask() in [TASK_ROLLBACK, TASK_ROLLBACK_PREREQ_CHECK] and len(_list_of_cells) > 0:
            '''
        ...
        if len(_list_of_cells) > 0:
            ...
            _key = self.mPrepareEnvironment(self.mGetDom0ToPatchcellSwitches(), _list_of_cells,
                                                           self.mGetCellSwitchesPatchBaseAfterUnzip(), PATCH_CELL)
            ...
            '''
            self.mUpdateServiceStateOnIlom(_list_of_cells, "prepatch")
            ...
            if _ret != PATCH_SUCCESS_EXIT_CODE: #Patch Failed
                ...
                _input_file = ""
                _reachableCellList = self.mCheckAndUpdateCellListPostPatchFailure(_list_of_cells,
                                        self.mGetCellSwitchesPatchBaseAfterUnzip(), self.mGetDom0ToPatchcellSwitches())
    ...
    def mCheckAndUpdateCellListPostPatchFailure(self, aCellList, aBaseDir, aDom0):

        _reachableCellList  = []
        for _aCell in aCellList:
            _cell = exaBoxNode(get_gcontext())
            if _cell.mIsConnectable(aHost=_aCell, aTimeout=mGetSshTimeout()):
                _reachableCellList.append(_aCell)
            else:
                self.mPatchLogError(f"Unable to connect to cell node : {str(_aCell)}.")
        
        return _reachableCellList
