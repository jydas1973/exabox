#
# $Header: ecs/exacloud/exabox/infrapatching/handlers/mockTargetHandler/switchmockhandler.py /main/3 2025/01/17 05:22:26 emekala Exp $
#
# switchmockhandler.py
#
# Copyright (c) 2020, 2024, Oracle and/or its affiliates.
#
#    NAME
#      switchmockhandler.py - Patch - Switch Basic Functionality
#    DESCRIPTION
...
class SwitchMockHandler(TargetMockHandler):

    def __init__(self, *initial_data, **kwargs):

        super(SwitchMockHandler, self).__init__(*initial_data, **kwargs)
        self.mPatchLogInfo("SwitchMockHandler")
    ...
    def mSetEnvironment(self):
        ...
        self.mGetExecutedTargets().append(PATCH_IBSWITCH)

        # Set collect time stats flag
        self.mSetCollectTimeStatsFlag(self.mGetCollectTimeStatsParam(PATCH_IBSWITCH))

    def mGetIBSwitchesList(self):

        # Get version details
        self.__ibswitch_upgrade_version, self.__ibswitch_rollback_version = self.mGetSwitchTargetVersion(PATCH_IBSWITCH)
        ...
        #self.__currentPATCH_IBSWITCH = PATCH_IBSWITCH
    ...
    def mCheckIBSwitchVersion(self, aIBSwitch, aVersionToCompare=None):
        ...
        self.mPatchLogInfo('mCheckIBSwitchVersion: Current version = %s, and Comparing Version = %s' %
    ...
    def mFilterIBSwitchesToPatch(self, aIBSwitchList):
        ...
        # Update status
        self.mUpdatePatchStatus(True, STEP_FILTER_NODES + "_" + PATCH_IBSWITCH)
    ...
    def mGatherIBSwitchPreCheckData(self, aIBSwitchesList):
        ...
        for _ibswitch in aIBSwitchesList:
            ...
            # Update status
            self.mUpdatePatchStatus(True, STEP_GATHER_NODE_DATA + "_" + PATCH_IBSWITCH)
    ...
    def mPreCheck(self):
        ...
        try:
            ...
            self.mPatchLogInfo(
                f"\n\n---------------> Starting {self.mGetTask()} in {PATCH_IBSWITCH} <---------------\n\n")
            ...
            self.mPatchLogInfo(f"Task: {_task_type} - Type: {PATCH_IBSWITCH}\t\t[ ret_code = {_ret} ]")
            if _ret != PATCH_SUCCESS_EXIT_CODE:
                self.mPatchLogError(f"{PATCH_IBSWITCH} '{_task_type}' failed. Patch execution stopped.")
            ...
            if _child_request_error_already_exists_in_db:
                ...
                _suggestion_msg = f"{PATCH_IBSWITCH} '{_task_type}' failed. Patch execution stopped."
    ...
    def mRollBackPreCheck(self):
        ...
        try:
            ...
            if _child_request_error_already_exists_in_db:
                ...
                _suggestion_msg = f"{PATCH_IBSWITCH} '{_task_type}' failed. Rollback Precheck execution stopped."
                self.mAddError(_ret, _suggestion_msg)
    ...
    def mPatch(self):
        ...
        try:
            ...
            self.mPatchLogInfo(f"Final return code from task : {self.mGetTask()} is {_ret} ")
            self.mPatchLogInfo(
                f"\n\n---------------> Completed {self.mGetTask()} on {PATCH_IBSWITCH}s <---------------\n\n")
            return _ret, _no_action_taken
    ...
    def mRollBack(self):
        ...
        try:
            ...
            self.mPatchLogInfo(f"Task: {PATCH_IBSWITCH} - Type: {PATCH_IBSWITCH}\t\t[ ret_code = {_ret} ]")
            if _ret != PATCH_SUCCESS_EXIT_CODE:
                self.mPatchLogError(f"{PATCH_IBSWITCH} '{_task_type}' failed. Patch execution stopped.")
            ...
            if _child_request_error_already_exists_in_db:
                ...
                _suggestion_msg = f"{PATCH_IBSWITCH} '{_task_type}' failed. Rollback execution stopped."
                self.mAddError(_ret, _suggestion_msg)
    ...
    def mCheckIdemPotency(self):
        ...
        self.mCleanEnvironment(self.mGetDom0ToPatchcellSwitches(), _list_of_ibswitches_retry_case,
                                ...
                                self.mGetPatchmgrLogPathOnLaunchNode(), PATCH_IBSWITCH, _exit_code)
    ...
    def mRegularPatchRun(self, aIBswitchesToUpgrade):
        ...
        self.mUpdatePatchProgressStatus(aNodeList=_list_of_ibswitches, aDiscardedNodeList=_discarded)
        ...
        if not self.mPatchRequestRetried() and len(_list_of_ibswitches) > 0 and _no_action_taken == 0:
            ...
            _key = self.mPrepareEnvironment(self.mGetDom0ToPatchcellSwitches(), _list_of_ibswitches,
                                                           self.mGetCellSwitchesPatchBaseAfterUnzip(), PATCH_IBSWITCH)
            ...
            if not self.mGetDom0ToPatchcellSwitches():
                _suggestion_msg = f"Launch node is either down or patches are not staged, unable to proceed with {PATCH_IBSWITCH} operation on {_task_type} target."
            ...
            self.mCleanEnvironment(self.mGetDom0ToPatchcellSwitches(), _list_of_ibswitches, _input_file,
                                    ...
                                    self.mGetPatchmgrLogPathOnLaunchNode(), PATCH_IBSWITCH, _ret)
    ...
    def mPostCheck(self):
        ...
        try:
            ...
            if _child_request_error_already_exists_in_db:
                ...
                _suggestion_msg = f"{PATCH_IBSWITCH} '{self.mGetTask()}' failed. Postcheck execution stopped."
                self.mAddError(_ret, _suggestion_msg)
    ...
    def mOneOff(self):
        ...
        _ret = PATCH_SUCCESS_EXIT_CODE
        self.mPatchLogInfo(f"\n\n---------------> Starting {TASK_ONEOFF} in {PATCH_IBSWITCH}s <---------------\n\n")
        try:
            ...
            if self.mGetPluginHandler() and self.mIsOneOffPluginEnabled():
                ...
                self.mGetPluginHandler().mSetPluginTarget(PATCH_IBSWITCH)
                ...
                _suggestion_msg = TASK_ONEOFF.upper() + " plugin is unavailable for " + PATCH_IBSWITCH.upper()
                self.mAddError(_ret, _suggestion_msg)
                raise self.mPatchLogError(TASK_ONEOFF.upper() +
                                          " plugin is unavailable for " + PATCH_IBSWITCH.upper())
        except Exception as e:
    ...
    def mDoIBSwitchPostCheck(self, aIBSwitchesData):
        ...
        for _ibswitch in aIBSwitchesData.keys():
            # Update status
            self.mUpdatePatchStatus(True, STEP_POSTCHECKS + "_" + PATCH_IBSWITCH)
            # Start check
